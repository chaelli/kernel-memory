...
public class HttpAddUrlRequest
{
        ...
        HttpRequest httpRequest, CancellationToken cancellationToken = default)
    {
        ...
        if (!form.TryGetValue(Constants.WebServiceDocumentIdField, out StringValues urlString) || urlString.Count != 1 || !Uri.TryCreate(urlString.FirstOrDefault(), UriKind.Absolute, out Uri url))
        {
            return (result, false, $"Invalid URL, '{Constants.WebServiceDocumentIdField}' must be a single valid URL");
        }
